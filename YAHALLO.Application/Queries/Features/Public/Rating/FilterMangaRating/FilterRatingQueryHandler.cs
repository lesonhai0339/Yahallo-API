    ...
    public class FilterRatingQueryHandler : IRequestHandler<FilterRatingQuery, PagedResult<RatingDto>>
    {
        ...
        public async Task<PagedResult<RatingDto>> Handle(FilterRatingQuery request, CancellationToken cancellationToken)
        {
            ...
            var ratings = await _ratingRepository
                ...
                request.PageSize,
                selector: q =>
                ApplySorting(ApplyFilter(q, request, targetId), request)
        ...
        private IQueryable<RatingEntity> ApplyFilter(IQueryable<RatingEntity> query, FilterRatingQuery request, string? targerId)
        {
            ...
            if (!string.IsNullOrEmpty(targerId)) query = query.Where(x => x.UserId == targerId);
            return query;
        }
    }
}

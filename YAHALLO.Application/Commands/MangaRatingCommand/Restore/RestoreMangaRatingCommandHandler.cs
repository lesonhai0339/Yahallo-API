...
{
    public class RestoreMangaRatingCommandHandler : IRequestHandler<RestoreMangaRatingCommand, bool>
    {
        ...
        public async Task<bool> Handle(RestoreMangaRatingCommand request, CancellationToken cancellationToken)
        {
            var checkMangaRatingExist = await _mangaRatingRepository
                .FindAsync(x => x.MangaId == request.MangaId && x.UserId == request.UserId, cancellationToken, ignoreQueryFilters: true);
